...
class QRData {
  ...
  static QRData? parse(String code) {
  ...
  static QRData? _parseJson(String code) {
    try {
  ...
  static QRData? _parsePipeSeparated(String code) {
    try {
  ...
  static QRData? _parseColonSeparated(String code) {
    try {
      final parts = code.split(':');
      if (parts.isEmpty) return null;
      ...
      // Check if first part is a prefix like "STUDENT" or "STU"
      if (parts[0].toUpperCase() == 'STUDENT' || 
          parts[0].toUpperCase() == 'STU' ||
      ...
      return QRData(id: id, generatedAt: generatedAt);
    } catch (e) {
  ...
  @override
  String toString() {
    return 'QRData(id: $id, name: $name, rollNumber: $rollNumber, isValid: $isValid)';
  }
}